# ============================================
# File: scripts/cards/build_cards.py
...
def build_all_cards(limit_posts: int | None = None) -> None:
    """Основной сценарий:
      - берем только 1 пост для тестов,
    ...
    print("[cards][build] === Старт генерации карточек (TEST MODE, только 1 пост) ===")

    # Берём только ОДИН пост для тестов
    ...
    print(f"[cards][build] Найдено постов: {len(posts)} (TEST MODE)")
    ...
    print("[cards][build] === Генерация карточек завершена (TEST MODE, 1 пост) ===")


if __name__ == "__main__":
    # Жёстко: всегда работаем в тестовом режиме → только 1 пост
    build_all_cards(limit_posts=1)
